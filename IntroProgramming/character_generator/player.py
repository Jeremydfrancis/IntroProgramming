...
def gen_name():
    """Generates a random name from first, middle, and last name lists."""
    ...
    full_name = pos1 + " " + pos2 + ", " + pos3
    return full_name


def gen_history():
    """Generates a random backstory and quest for the player using mad-lib style templates."""
...
def gen_player(level):
    """Generates player dictionary with randomized stats based on the given level."""
    player = {"name": gen_name(),
              "attack": my_utilities.roll_dice(3, 6) + my_utilities.roll_dice(level, 4),
              "defense": my_utilities.roll_dice(3, 6) + my_utilities.roll_dice(level, 4),
              "health": my_utilities.roll_dice(5, 6) + my_utilities.roll_dice(level, 4),
              "row":0,
              "column":0,
              "treasure_found": [],
              "history": gen_history()}
...
def gen_enemy(level):
    """Generates enemy dictionary with randomized stats based on the given level."""
    enemy = {"name": gen_name(),
             "attack": my_utilities.roll_dice(3, 6) + my_utilities.roll_dice(level, 4),
             "defense": my_utilities.roll_dice(3, 6) + my_utilities.roll_dice(level, 4),
             "health": my_utilities.roll_dice(5, 6) + my_utilities.roll_dice(level, 4),}
...
def generate_char_info(player):
    """Displays generated character info and lets the user accept or generate a different character"""
    level = 0
    if level == 0:
        level = 1
    else:
        level = len(player["treasure_found"])
    wrapper = textwrap.TextWrapper(width=80)
    while True:
        player = gen_player(level)
        print("NAME:\t" + player["name"] )
        for key in player:
            if key != "name" and key != "history" and key != "treasure_found" :
                print(
                    wrapper.fill(key.upper() + ":" + " " + str(player[key] )),
                    end="\t"
                )
        print()
        print("\n"+wrapper.fill("HISTORY: " + player["history"]))

        user_input = input("\nGenerate another character (Y/N)? ").upper()
        if user_input == "N":
            return player
        else:
